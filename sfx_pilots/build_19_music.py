"""$0 Cinematic-Orchestral music bed for '19 The Cliff of Rival Gods' — chains two approved
music_library Suno tracks under viral_cut_sfx.mp4 (no metered Eleven Music).

Arc: lonely_searching (the cold cliff of dead gods, the searching/undecided) crossfading into
sacred_grace_rise, whose swell (track 140-160s = the loudest crest) is sliced so the crescendo
LANDS on the living Christ / the hero close ('Father, open my eyes to your Son'). -11 dB, ducked
under the voice (ratio 5) so narration stays clear over the climax. No choir pad (SFX/score only).
"""
import subprocess
...
ASM = Path(r"C:\Users\sanjay\PycharmProjects\PythonProject1\jesus\narration"
           r"\19 The Cliff of Rival Gods\v1\assembly")
...
fc = (
    f"[1:a]{AF},atrim=0:32,asetpts=PTS-STARTPTS[s0];"          # lonely_searching — the dead-gods hook
    f"[2:a]{AF},atrim=130:168,asetpts=PTS-STARTPTS[s1];"        # sacred_grace_rise — the swell to the crest
...
print(f"[music] chaining lonely_searching -> sacred_grace_rise swell under {SRC.name} ({D:.1f}s)")
