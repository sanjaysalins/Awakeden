...
def _scene_plan_json_contract(beat_ids: Iterable[str]) -> str:
    ...
    return (
        ...
        '      "shot_kind": "standard | hero | insert   (hero = the gospel-pivot image that CLOSES the final cut as the CTA landing, so the Short lands on Christ — make it iconic, near-still, reverent; insert = a single-macro ~2s shot purpose-built for a TINY narration beat, see the NARRATION BOARD; standard = everything else)"\n'
        "    }\n"
