...
- G1 Biblical Accuracy: the quoted verse matches the verified KJV verbatim — EXACT wording AND exact terminal punctuation (a quote ending '!' in the KJV must not become '?' or '.'); the reference is correct; the Point/claim is sound when read in the wider pericope (no proof-texting, no over-claim). FAIL (not CONDITIONAL) on any statement that is exegetically FALSE or that a literate skeptic could disprove from the text (e.g. a strawman rebuttal, an over-tidy historical claim) — a false aside is worse than no aside. ALSO FAIL a SCENE-SCOPE OVERCLAIM: a claim that asserts a divine attribute or act the SCENE only implies (e.g. 'the One who made it' / Creator where the text shows only authority/lordship; a pre-resurrection text leaning on 'He is risen / the empty tomb'). True in the wider canon is NOT enough — the line must be airtight to what THIS pericope establishes; tighten the wording to the scene. ALSO FAIL an INVENTED NARRATIVE DETAIL: a NON-quoted claim asserted as FACT about a Bible character's actions, whereabouts, or timeline that the biblical text does not support or contradicts (e.g. 'Peter watched the scourging' — Scripture has Peter at the high priest's courtyard denying Christ and gone before the Roman scourging). A vivid detail invented to heighten drama is freshness-by-invention. This is narrow: a claim the text DOES support (e.g. 'John watched it at the cross', John 19:26) is fine — only FAIL claims that are unsupported or contradicted by the record.
