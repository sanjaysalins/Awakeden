"""Look and Live -- step 3b: overlay the LOCKED sketchbook title/verse-card
standard (yellow/black title, red/white-outline quote, red/cream citation)
onto the already-assembled core cut. Ported verbatim from
batches/cluster_01_cross/forsaken_cry_ps221/_poc4_full_standard.py's
type_img() -- see memory sketchbook-title-verse-card-standard-LOCKED-2026-08-11.

Reads LOOKANDLIVE_living_sketchbook.mp4, overlays the cards, writes back to
the same filename (via a temp intermediate).

  .venv\\Scripts\\python.exe poc_living_sketchbook/look_and_live/_s3b_titlecards.py
"""
import math
import subprocess
import sys
...
def main():
    ...
    final = HERE / "LOOKANDLIVE_living_sketchbook.mp4"
    backup = HERE / "_LOOKANDLIVE_prewatermark_nocards.mp4"
