"""Even So Must the Son of Man Be Lifted Up -- step 3b: overlay the LOCKED
sketchbook title/verse-card standard (yellow/black title, red/white-outline
quote, red/cream citation) onto the already-assembled core cut. Ported
verbatim from look_and_live/_s3b_titlecards.py -- same locked standard,
see memory sketchbook-title-verse-card-standard-LOCKED-2026-08-11.

Reads SONOFMANLIFTEDUP_living_sketchbook.mp4, overlays the cards, writes
back to the same filename (via a temp intermediate).

Second quote+citation pair sits over s05/s06's own window (28.4-32.8s) --
the exact moment the narration itself speaks that half of the KJV quote
(John 3:15), same "card reinforces the spoken line" pattern Look and Live's
own s06 used.

  .venv\\Scripts\\python.exe poc_living_sketchbook/son_of_man_lifted_up/_s3b_titlecards.py
"""
import math
import subprocess
import sys
...
SRC = HERE / "SONOFMANLIFTEDUP_living_sketchbook.mp4"
...
# Real bug found 2026-08-13 (user caught it): type_img() had no width ceiling
# at all -- a long quote line just rendered at whatever width the text
# needed, so on a 1080px-wide 9:16 frame a 28+ character line could exceed
# the frame and get clipped off both edges. Shrink-to-fit keeps every card
# safely inset (mirrors the Noah caption's own "never wall-to-wall" MAX_
# TEXT_W discipline, applied here to the title/quote/citation layer instead
# of the spoken-caption layer). MAX_CARD_W leaves rotation headroom (the
# card gets rotate(-1.5) after this, which grows its bounding box a little).
...
# pattern as Look and Live/God Hung Up a Snake. Second quote+citation pair
# sits over s05/s06's own window (28.4-32.8s), the exact moment the
# narration itself speaks that half of the KJV quote.
CARD_DEFS = [
    (0.3, 4.0, "hilite", "EVEN SO MUST THE SON\nOF MAN BE LIFTED UP.", 0.50, 0.09),
    (4.6, 8.9, "quote", "AND AS MOSES LIFTED UP\nTHE SERPENT, EVEN SO MUST\nTHE SON OF MAN BE LIFTED UP:", 0.50, 0.20),
    (4.9, 8.9, "card", "JOHN 3:14", 0.50, 0.40),
    (28.4, 32.8, "quote", "THAT WHOSOEVER BELIEVETH\nIN HIM SHOULD NOT PERISH,\nBUT HAVE ETERNAL LIFE.", 0.50, 0.15),
    (28.7, 32.8, "card", "JOHN 3:15", 0.50, 0.35),
]
...
def main():
    ...
    final = HERE / "SONOFMANLIFTEDUP_living_sketchbook.mp4"
    backup = HERE / "_SONOFMANLIFTEDUP_prewatermark_nocards.mp4"
