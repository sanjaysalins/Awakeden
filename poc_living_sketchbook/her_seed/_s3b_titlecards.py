"""Her Seed -- step 3b: overlay the LOCKED sketchbook title/verse-card
standard (yellow/black title, red/white-outline quote, red/cream citation)
onto the already-assembled core cut. Ported verbatim from the sibling
short's own _s3b_titlecards.py -- same locked standard, INCLUDING the
2026-08-13 shrink-to-fit width-safety fix (MAX_CARD_W) from day one.

Reads HERSEED_living_sketchbook.mp4, overlays the cards, writes back to
the same filename (via a temp intermediate).

Quote+citation pair sits over s04's own window (20.39-27.877s) -- the
exact moment the narration speaks the piece's one literal KJV quote
(Galatians 4:4's "made of a woman, made under the law").

  .venv\\Scripts\\python.exe poc_living_sketchbook/her_seed/_s3b_titlecards.py
"""
import math
import subprocess
import sys
...
SRC = HERE / "HERSEED_living_sketchbook.mp4"
...
def main():
    ...
    final = HERE / "HERSEED_living_sketchbook.mp4"
    backup = HERE / "_HERSEED_prewatermark_nocards.mp4"
