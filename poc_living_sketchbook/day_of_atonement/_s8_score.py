"""Day of Atonement LONG -- step 8: SCORE. Reuses the SAME proven Suno recipe
already locked for this exact story's other treatment
(longform/_add_score_lf.py's EPISODES["06_Day_Of_Atonement"], verbatim) --
same segments/xfade/gain/outro, just this sketchbook pilot's own base cut as
src/out. Same pattern bronze_serpent_long/_s8_score.py used for its own
story's reuse.
...
narration-pad + duck + mix tail), same one every other long-form scorer uses.
  ...
  .venv\\Scripts\\python.exe poc_living_sketchbook/day_of_atonement/_s8_score.py --yes
"""
import argparse
import subprocess
import sys
...
SRC = HERE / "DAYOFATONEMENT_LONG_living_sketchbook.mp4"
OUT = HERE / "DAYOFATONEMENT_LONG_living_sketchbook_scored.mp4"
...
def run(yes: bool, regen: bool) -> None:
    if not SRC.exists():
        sys.exit(f"missing base cut: {SRC} -- run _s6_assemble.py first")
