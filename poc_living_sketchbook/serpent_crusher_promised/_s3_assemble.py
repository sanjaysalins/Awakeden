"""The Serpent-Crusher Promised (Seed of the Woman short #4) -- step 3:
...
windows from `_alignment.json` (134/134 words matched).
...
across its real window. Hard cuts throughout.
  ...
  .venv\\Scripts\\python.exe poc_living_sketchbook/serpent_crusher_promised/_s3_assemble.py --test-window 15 19
  .venv\\Scripts\\python.exe poc_living_sketchbook/serpent_crusher_promised/_s3_assemble.py
"""
import argparse
import shutil
import subprocess
...
SRC_AUDIO = (Path(__file__).resolve().parents[3] / "PythonProject1" / "jesus" /
             "narration" / "46_Old_Story._Unfinished" / "v1" / "narration.mp3")
OUT = HERE / "SERPENTCRUSHERPROMISED_living_sketchbook.mp4"

W, H, FPS = 1080, 1920, 30
# 2026-08-15: re-timed after adding a distinct "scripture" voice for the
# Romans 16:20 quote (per the user's own call, matching Her Seed's own
# precedent) -- narration.mp3 was re-synthesized, alignment shifted throughout.
LAST_WORD_END = 58.863  # "feet." real offset, _alignment.json (was 57.153)
HOLD = 3.137             # >= INV-26's 3.0s minimum hold, lands TOTAL on a clean number
...
SHOTS = [
    ("s01_recap_curse", 0.00, 5.740),
    ("s02_unfinished_arch", 5.740, 7.950),
    ("s03_armor_set_aside", 7.950, 15.300),
    ("s04_pauls_letter", 15.300, 20.139),
    ("s05_feet_on_crushed_head", 20.139, 30.459),
    ("s06_empty_cross_shadow", 30.459, 39.305),
    ("s07_night_watchman", 39.305, 49.767),
    ("s08_gold_thread_bridge", 49.767, 55.952),
    ("s09_landing_christ_in_arch", 55.952, TOTAL),
]
...
def main():
    ...
    print("[extract] pulling frame sequences from all 9 real clips ...")
