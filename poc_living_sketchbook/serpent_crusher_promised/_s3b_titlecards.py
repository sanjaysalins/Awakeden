"""The Serpent-Crusher Promised -- step 3b: overlay the LOCKED sketchbook
title/verse-card standard (yellow/black title, red/white-outline quote,
red/cream citation) onto the already-assembled core cut. Ported from the
sibling shorts' own _s3b_titlecards.py -- same locked standard, including
the shrink-to-fit width-safety fix (MAX_CARD_W).

Reads SERPENTCRUSHERPROMISED_living_sketchbook.mp4, overlays the cards,
writes back to the same filename (via a temp intermediate).

Quote+citation pair sits over s05's own HERO window (19.83-29.008s) -- the
exact moment the narration speaks the KJV quote (Romans 16:20's "the God
of peace shall bruise Satan under your feet shortly").

  .venv\\Scripts\\python.exe poc_living_sketchbook/serpent_crusher_promised/_s3b_titlecards.py
"""
import math
import subprocess
import sys
...
SRC = HERE / "SERPENTCRUSHERPROMISED_living_sketchbook.mp4"
...
# added for the KJV quote shifted every downstream timestamp).
CARD_DEFS = [
    (0.3, 4.0, "hilite", "THE SERPENT-CRUSHER PROMISED.", 0.50, 0.09),
    (20.4, 30.7, "quote", "AND THE GOD OF PEACE\nSHALL BRUISE SATAN\nUNDER YOUR FEET SHORTLY.", 0.50, 0.15),
    (20.7, 30.7, "card", "ROMANS 16:20", 0.50, 0.40),
]
...
def main():
    ...
    final = HERE / "SERPENTCRUSHERPROMISED_living_sketchbook.mp4"
    backup = HERE / "_SERPENTCRUSHERPROMISED_prewatermark_nocards.mp4"
