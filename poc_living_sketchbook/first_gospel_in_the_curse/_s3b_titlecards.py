"""The First Gospel in the Curse -- step 3b: overlay the LOCKED sketchbook
title/verse-card standard (yellow/black title, red/white-outline quote,
red/cream citation) onto the already-assembled core cut. Ported verbatim
from the Bronze Serpent shorts' own _s3b_titlecards.py -- same locked
standard, INCLUDING the 2026-08-13 shrink-to-fit width-safety fix
(MAX_CARD_W) from day one this time, not retrofitted after a clipping bug.

Reads FIRSTGOSPELINTHECURSE_living_sketchbook.mp4, overlays the cards,
writes back to the same filename (via a temp intermediate).

Second quote+citation pair sits over s05's own window (24.6-29.6s) -- the
exact moment the narration itself speaks that half of the KJV quote
(Genesis 3:15's "bruise thy head... bruise his heel").

  .venv\\Scripts\\python.exe poc_living_sketchbook/first_gospel_in_the_curse/_s3b_titlecards.py
"""
import math
import subprocess
import sys
...
# 1080px frame and clip off both edges (real bug found 2026-08-13 on 3
# other shorts, see memory sketchbook-title-verse-card-standard-LOCKED).
...
CARD_DEFS = [
    (0.3, 4.0, "hilite", "THE FIRST GOSPEL\nIN THE CURSE.", 0.50, 0.09),
    (4.6, 8.9, "quote", "AND I WILL PUT ENMITY\nBETWEEN THEE AND\nTHE WOMAN...", 0.50, 0.20),
    (4.9, 8.9, "card", "GENESIS 3:15", 0.50, 0.40),
    (24.6, 29.6, "quote", "IT SHALL BRUISE THY HEAD,\nAND THOU SHALT BRUISE\nHIS HEEL.", 0.50, 0.15),
    (24.9, 29.6, "card", "GENESIS 3:15", 0.50, 0.35),
]
...
def main():
    ...
    final = HERE / "FIRSTGOSPELINTHECURSE_living_sketchbook.mp4"
    backup = HERE / "_FIRSTGOSPELINTHECURSE_prewatermark_nocards.mp4"
