...
every other Bronze Serpent short's own _s0_align.py. $0 local forced-
alignment MEASUREMENT pass over an already-locked, already-voiced
narration -- no API spend, no text changes.

Source narration folder title is "Grace Spoken First"
(PythonProject1/jesus/narration/44_.../) -- same piece, different working
title than the manifest's own "The First Gospel in the Curse."

  .venv\\Scripts\\python.exe poc_living_sketchbook/first_gospel_in_the_curse/_s0_align.py
"""
import json
import re
import sys
...
SRC = (Path(__file__).resolve().parents[3] / "PythonProject1" / "jesus" /
       "narration" / "44_Grace_Spoken_First" / "v1")
