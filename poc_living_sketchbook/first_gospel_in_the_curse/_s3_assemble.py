"""The First Gospel in the Curse (Seed of the Woman short #1) -- step 3:
assemble the CORE cut (visual + real narration audio only -- score/sfx/
captions/watermark are separate follow-up stages). Real word-timed spread
windows from `_alignment.json` (forced WhisperX alignment against the
locked narration.mp3, 165/165 words matched).

Architecture ported directly from the Bronze Serpent shorts' own
_s3_assemble.py (same episode family, same style): ffmpeg extracts every
clip to its own frame sequence once, a ping-pong helper holds/bounces each
clip's frames across its real window. Hard cuts throughout.

  .venv\\Scripts\\python.exe poc_living_sketchbook/first_gospel_in_the_curse/_s3_assemble.py --test-window 15 19
  .venv\\Scripts\\python.exe poc_living_sketchbook/first_gospel_in_the_curse/_s3_assemble.py
"""
import argparse
import shutil
import subprocess
...
SRC_AUDIO = (Path(__file__).resolve().parents[3] / "PythonProject1" / "jesus" /
             "narration" / "44_Grace_Spoken_First" / "v1" / "narration.mp3")
OUT = HERE / "FIRSTGOSPELINTHECURSE_living_sketchbook.mp4"

W, H, FPS = 1080, 1920, 30
LAST_WORD_END = 65.705   # "first." real offset, _alignment.json
HOLD = 3.295              # >= INV-26's 3.0s minimum hold, small safety margin
TOTAL = 69.0              # LAST_WORD_END + HOLD, frame-exact at 30fps (2070 frames)

# (name, t0, t1) -- real word-timed windows built against _alignment.json,
# matching _PLAN.md's spread table.
SHOTS = [
    ("s01_hook_hands", 0.00, 4.58),
    ("s02_waiting_in_trees", 4.58, 9.26),
    ("s03_turns_to_serpent", 9.26, 18.42),
    ("s04_serpent_in_light", 18.42, 24.31),
    ("s05_heel_and_head_insert", 24.31, 29.85),
    ("s06_turn_to_eve_adam", 29.85, 35.27),
    ("s07_gold_thread_in_curse", 35.27, 42.94),
    ("s08a_eve_face_conviction", 42.94, 48.26),
    ("s08b_open_hands", 48.26, 53.26),
    ("s09_landing_transition", 53.26, 59.77),
    ("s10_landing_christ", 59.77, TOTAL),
]
...
def main():
    ...
    print("[extract] pulling frame sequences from all 11 real clips ...")
