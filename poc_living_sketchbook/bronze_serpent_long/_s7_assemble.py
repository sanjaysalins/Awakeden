"""Bronze Serpent LONG -- step 7: SIMPLE FIRST CUT. Turns `_spread_windows.json`
(68 spreads, each with a resolved fill mode) into one silent 1920x1080@30fps
video, then muxes the real locked narration on top with the INV-26 landing
hold. Hard cuts between spreads (no crossfade/page-turn devices) -- those are
a deliberate polish-pass-2 layer added AFTER this base cut is seen and
approved, per the user's own 2026-08-02 "simple first, polish after" call.
Nothing here is generative -- every segment is built by ffmpeg trim/loop/
reverse/concat over already-approved clips and stills.

Fill modes (resolved per-spread in `_spread_windows.json` by
`_s6b_spread_windows.py`):
  once_trim    -- clip is >= window: play from its start, trimmed to fit.
  once_hold    -- clip already AT the window duration (the 10 deterministic
                  spreads): play as-is, pad any sub-frame remainder by
                  holding the last frame (ffmpeg tpad).
  ...
  fwd_tail_bounce -- directional motion (walking/falling/rising, see
                  _s6b's ONE_WAY set): play the clip forward ONCE at native
                  speed (so the real motion is seen the right way round),
                  then fill the remainder by bouncing only a short CALM
                  TAIL -- never a full-clip reverse.
  static_still -- s68 landing: no clip, hold the still image for the whole
                  window (the tear_hole reveal is a polish-pass device;
                  here it's just a plain held frame).

  .venv\\Scripts\\python.exe poc_living_sketchbook/bronze_serpent_long/_s7_assemble.py               # full 68-spread build
  .venv\\Scripts\\python.exe poc_living_sketchbook/bronze_serpent_long/_s7_assemble.py --only s01_wide,s30_payoff_fever_breaks   # test gate
  .venv\\Scripts\\python.exe poc_living_sketchbook/bronze_serpent_long/_s7_assemble.py --concat-only   # (re)join existing segments + mux audio
"""
import argparse
import json
import subprocess
...
NARRATION = HERE.parents[1] / "longform" / "EW04_Bronze_Serpent" / "v1" / "narration.mp3"
...
OUT_SILENT = HERE / "_BRONZESERPENT_LONG_silent.mp4"
OUT_CUT = HERE / "BRONZESERPENT_LONG_living_sketchbook.mp4"
...
def main():
    ...
    if not WINDOWS.exists():
        print(f"[FATAL] {WINDOWS} not found -- run _s6b_spread_windows.py first")
        sys.exit(1)
