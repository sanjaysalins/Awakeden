...
| LF-INV-11 | **Legacy-oil → ink migration recipe** (locked 2026-07-21; precedents: Bronze Serpent 2026-07-16, EW01 Two Goats 2026-07-21). Migrating an archived Baroque-oil episode to the inked graphic-novel look follows this exact path, in order: **(1) Port the plan** — a per-episode `_build_inked_scene_plan.py` reads the ARCHIVED oil `scene_plan.json` and writes `v1/visual_16x9_inked/scene_plan.json`; scene content/camera/timing unchanged, style comes from `config.STYLE_REGISTRY` at render time. **(2) Redesign volatile stills at the plan stage** — any mid-action liquid/motion content (sprinkling blood, falling drops, mid-swing tools) is rewritten to a SETTLED, completed state BEFORE rendering: every i2v model completes mid-action into invented motion (proven by the 7-attempt nail_through_hand failure + the clip-QC repair batch). **(3) Test-gate (LF-INV-7)** — 1–2 paid stills, user look-lock, then the batch. **(4) Full-set eye-audit** — every still is viewed at full res before animation; known ink-model failure modes to check: Christ's nail scars rendered as barbed star-scribbles (fix: positive-only "single small quiet healed nail scar in each palm" phrasing — seedream has no negative channel), witness/world identity drift (hair/age), period-wrong architecture or idols (lamassu, steeples, rooftop crosses), and any liquid drawn mid-drip. Fixes go into the plan builder (append/replace maps), never hand-edited into one render. **(5) Stills human gate → tiered animation** (Seedance calm / Kling action, INV-13 camera-only) → clip QC with sidecars → assembly → score/sfx → INV-26 hold → INV-27 watermark. |
