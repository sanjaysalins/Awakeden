...
def render_move(still: Path, move: str, dur: float, focus, dest: Path, amp: float = 1.0) -> Path:
    if move == "tour":
    ...
    for i in range(n):
        ...
        if move == "arc":
            yaw = math.radians(7.0 * amp) * (2 * e - 1)     # sweep -7deg*amp -> +7deg*amp
            pitch, scale = 0.0, 1.0 + 0.10 * amp * e
        elif move == "swoop":
            yaw = math.radians(4.0 * amp) * (2 * e - 1)
            pitch = math.radians(5.0 * amp) * (e - 0.5)     # slight craning pitch
            scale = 1.0 + 0.16 * amp * e
        else:                                           # push (baseline)
            yaw = pitch = 0.0; scale = 1.0 + 0.12 * amp * e
