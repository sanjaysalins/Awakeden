...
def _dims(path):
    """Real (w,h) of a video source, ffprobed once and cached. REUSE FIX (2026-07-16):
    fill_each panel-fit used to assume every source was PAGE-shaped (true for this
    episode's own 16:9 stills/Kling clips), which mis-solves the crop for a REUSED 9:16
    clip_library clip dropped into a two_v/triptych_v column. Probing the actual file is
    backward-compatible (own sources ARE page-shaped, so this returns PAGE for them)."""
    p = str(path)
