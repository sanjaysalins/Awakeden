...
def dyncam_clip(slug: str, move: str, dur: float = None) -> Path:
    """MOTION-FLOOR FIX (2026-07-16): a dyncam clip used to always render at a fixed
    5.0s regardless of how long the panel actually stays on screen; build_livingpage's
    _temporal() then setpts-STRETCHES that same fixed 5s sweep (7deg yaw / 10-16% zoom)
    across the whole beat. On a 20-30s beat that spreads an already-small sweep so thin
    that any few-second sample looks frozen (measured: <2% mean pixel diff over 1.5s on
    a 30s beat) -- exactly the "static full-bleed hold" defect flagged in review. Fix:
    render length tracks the beat's actual duration (capped, so render time stays
    bounded) AND the sweep amplitude scales up on long holds, so a fixed *fraction* of
    the beat always shows real motion instead of a near-flat creep."""
    if dur is None:
        render_len, amp = 5.0, 1.0
    else:
        render_len = min(max(dur, 3.0), 10.0)
        amp = min(2.6, 1.0 + max(0.0, dur - 6.0) / 10.0)
    dest = DYN / f"{slug}_{move}_{render_len:.1f}_{amp:.2f}.mp4"
    src = POOL / f"{slug}.png"
    ...
    return dc.render_move(POOL / f"{slug}.png", move, render_len, focus, dest, amp=amp)
