...
2. **The Hem — expand to the first full series episode.** Wrap ~4 new pages around the validated F04/F05 pair (the 12 years, the press of crowd, "Who touched my clothes?", the landing), write and lock the narration (hand-rolled scripts per the decision above, not a slash-command pipeline), then voice + assemble + score + caption using the existing `northstar_shortform/`-style assemble script as the template. This is the "ship one complete episode of the new series" milestone, on the most-tested ground we have. *Proves: full-episode Stain arc, hand-rolled end-to-end.* ~$20-30 (art/audio only, + ~$5-6 narration). **[OPEN — NEEDS YOUR CALL]** grok's review flagged that this episode's own guardrail may conflict with itself: the series' `guardrails` string says never assign Stain (sin/guilt) to suffering the text doesn't call sin (John 9:3), but The Hem's 12-year hemorrhage is ritual *uncleanness* (Leviticus 15), not stated moral guilt — and this is the flagship, most-tested Stain story in the whole slate. This is a real doctrinal-grounding question, not a mechanical bug, and per this project's own non-negotiable rule it needs your own check alongside mine before build 2 spends on it as the Stain's "home story." My own read: uncleanness-under-the-law and guilt are related-but-distinct categories in the text (the woman is never called a sinner; her healing removes a state, not a debt) — which would argue for reframing The Hem's motif language ("dried at the touch of the Law's own barrier," not "guilt dried") rather than dropping the episode. But that's my read, not a ruling — flagging for you, not deciding it here.
...
| **[OPEN — NEEDS YOUR CALL] Mark 5 Stain vs. the John 9:3 guardrail** | The Hem (build 2, the flagship Stain episode) assigns Stain to ritual uncleanness, not text-stated sin — possibly in tension with the plan's own guardrail. Flagged in build 2 above; needs your doctrinal read before that build spends, not just mine. |
