#!/usr/bin/env python
"""Pierced (Zech 12:10) — $0 score: lonely_searching_a -> sacred_grace_rise_a, grace landing on
the border-break ("I will pour", 38.1s). Sidechain-ducked under narration+slams; dips under the
Zechariah bar (12.1-18.5) and thinned under the CTA (48.9+). Mix follows the locked audio stack."""
...
OUT = HERE / "visual" / "pierced_zech1210_scored.mp4"
DARK, GRACE = MUS / "lonely_searching_a.mp3", MUS / "sacred_grace_rise_a.mp3"
TOTAL = 59.04 + 1.5                      # +1.5s outro hold on the risen hand

fc = (
    # music: dark 0-35s, crossfade 6s into grace (grace arrives ~38 = the pour)
    f"[1:a]atrim=0:38,aformat=sample_fmts=fltp:channel_layouts=stereo:sample_rates=44100[d];"
    ...
    f"volume=volume=0.35:enable='between(t,12.06,18.46)',"      # clear the room for the Zech bar
    f"volume=volume=0.5:enable='between(t,48.87,{TOTAL})'[mus];"  # thin under the CTA
