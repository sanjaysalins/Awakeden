#!/usr/bin/env python
"""Woman Behold Thy Son (John 19:26) — $0 score: lonely_searching_a -> sacred_grace_rise_a,
grace landing on the border-break ('Woman, behold thy son!', 28.8s). Ducked under narration;
dips under the Simeon sword bar and the 19:26a seeing bar; thinned under the CTA."""
...
OUT = HERE / "visual" / "woman_behold_john1926_scored.mp4"
DARK, GRACE = MUS / "lonely_searching_a.mp3", MUS / "sacred_grace_rise_a.mp3"
TOTAL = 59.06 + 1.5                      # +1.5s outro hold on the mercy hand

fc = (
    f"[1:a]atrim=0:28.7,aformat=sample_fmts=fltp:channel_layouts=stereo:sample_rates=44100[d];"
    ...
    f"volume=volume=0.35:enable='between(t,4.18,7.84)',"        # clear the room: Simeon's sword bar
    f"volume=volume=0.4:enable='between(t,22.09,31.87)',"       # 19:26a seeing bar -> 'Woman, behold thy son!'
    f"volume=volume=0.5:enable='between(t,53.16,{TOTAL})'[mus];"  # thin under the CTA
