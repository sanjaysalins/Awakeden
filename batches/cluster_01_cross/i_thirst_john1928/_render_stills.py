#!/usr/bin/env python
"""I Thirst (John 19:28 / Ps 22:15) — 3 new 9:16 stills via BytePlus (~$0.15). Lint-gated. Idempotent."""
...
JOBS = {
 "ocean_creation_wide": ("vast ocean waves rolling under a golden dawn sky, spray catching the light, no land, no figures, wide, vertical", None),
 "potsherd_dry_clay": ("a broken shard of dry clay pottery lying on cracked parched earth, harsh dry light, close, vertical, ancient Judea", None),
 "living_water_stream": ("a clear spring of water pouring over smooth rocks, sunlight sparkling in the falling water, green moss, close, vertical", None),
}
...
if not a.render:
    print("\n$0 dry-run. --render to spend (~$0.15)."); sys.exit(0)
