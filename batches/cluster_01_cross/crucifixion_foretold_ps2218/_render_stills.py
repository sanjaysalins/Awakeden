#!/usr/bin/env python
"""Crucifixion Foretold (Ps 22:18) ??? 1 new 9:16 still via BytePlus (~$0.05). Lint-gated. Idempotent.
david_writing_psalm is cluster-SHARED (forsaken_cry + i_thirst reuse it)."""
...
JOBS = {
 "david_writing_psalm": ("an aged king with a grey beard writing on an open scroll by a clay oil lamp, a harp resting against the stone wall beside him, night chamber, warm lamplight, vertical, ancient Israel", None),
 "lots_cup_close": ("two small carved knucklebone lots tumbling from a leather cup onto flat stone, a folded woven garment lying beside them, harsh daylight, close, vertical, 1st-century Judea", None),
}
