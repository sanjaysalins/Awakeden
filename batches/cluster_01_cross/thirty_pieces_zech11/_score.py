#!/usr/bin/env python
"""Thirty Pieces (Zech 11) — $0 score: lonely_searching_a -> sacred_grace_rise_a, grace landing
on the border-break ('The cross was no accident', 45.7s). Sidechain-ducked under narration;
dips under the Zech 11:12b and 11:13b bars; thinned under the CTA. Pilot mix stack."""
...
OUT = HERE / "visual" / "thirty_pieces_zech11_scored.mp4"
DARK, GRACE = MUS / "lonely_searching_a.mp3", MUS / "sacred_grace_rise_a.mp3"
TOTAL = 59.08 + 1.5                      # +1.5s outro hold on the risen hand

fc = (
    f"[1:a]atrim=0:45.6,aformat=sample_fmts=fltp:channel_layouts=stereo:sample_rates=44100[d];"
    ...
    f"volume=volume=0.35:enable='between(t,9.60,12.54)',"       # clear the room: 11:12b bar
    f"volume=volume=0.35:enable='between(t,23.63,28.39)',"      # clear the room: 11:13b bar
    f"volume=volume=0.5:enable='between(t,54.75,{TOTAL})'[mus];"  # thin under the CTA
