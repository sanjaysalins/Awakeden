#!/usr/bin/env python
"""The Forsaken Cry (Ps 22:1 / Matt 27:46) — $0 score: lonely_searching_a -> sacred_grace_rise_a,
grace entering on 'no cry of lost faith' (34.4s). Ducked under narration; dips under the two
identical-words bars; thinned under the CTA. Pilot mix stack."""
...
OUT = HERE / "visual" / "forsaken_cry_ps221_scored.mp4"
DARK, GRACE = MUS / "lonely_searching_a.mp3", MUS / "sacred_grace_rise_a.mp3"
TOTAL = 58.18 + 1.5                      # +1.5s outro hold on the dawn cross

fc = (
    f"[1:a]atrim=0:34.3,aformat=sample_fmts=fltp:channel_layouts=stereo:sample_rates=44100[d];"
    ...
    f"volume=volume=0.35:enable='between(t,17.82,22.6)',"
    f"volume=volume=0.35:enable='between(t,27.15,31.9)',"
    f"volume=volume=0.5:enable='between(t,51.7,{TOTAL})'[mus];"
