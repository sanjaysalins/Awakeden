"""Comic Page Pipeline POC -- Rung 2 v2 "LIVING COMIC" compositor.

v2.1 (user feedback 2026-07-26 on the first v2 cut):
  + PANEL-ANCHORED CAPTION BOXES -- the narration rides the page as parchment
    comic captions (verbatim spoken words, phrase-timed from the alignment);
    the two John 6:37 quote spans render RED-LETTER, anchored page-bottom.
    NO speech bubbles ever (user-locked).
  + COMIC-BOOK PAGE DRESS -- outer paper margin around the whole grid +
    soft drop shadow under every panel (paste-up feel). A print-grade
    halftone pass runs on the assembled cut (see _assemble_final_v2.py).
  + p5b re-extended forward-crossfade (its hand-off motion boomeranged into
    an endless scroll rock -- directional motion must never boomerang).

v2 core (unchanged): panels SLAM in word-timed, live panel bright / others
dimmed, ink-bleed page turns, full-bleed SPLASH on the IN NO WISE pivot,
page 5 = sacred stillness. Geometry/dim/border math copied from
panel_animator/grid_choreography.py so framing matches the approved v1.

  .venv\\Scripts\\python.exe poc_comic_page/rung2/_compose_pages_v2.py page1
  .venv\\Scripts\\python.exe poc_comic_page/rung2/_compose_pages_v2.py        # all, in order
"""
...
W, H = 1080, 1920
SS = 1.18
MARGIN = 26          # outer paper page border (screen px)
GUTTER = 12
BORDER_PX = 5
PAPER = (232, 217, 181)
INK = (35, 31, 32)
...
# captions: (panel_idx, anchor, text, t_in, t_out, red)
# anchor: 'tl'/'bl' = inside that panel corner; 'page_bottom' = centered strip
# above the bottom page margin (used for the two red-letter John 6:37 spans).
# Verbatim spoken words, phrase-timed from narration.alignment.json.
PAGES = {
    ...
    "page3": dict(clips=[EXT / "panel_b.mp4", R1_EXT / "panel_a.mp4",
    ...
    "page5": dict(clips=[EXT / "p5a.mp4", EXT / "p5b.mp4", EXT / "p5c.mp4"],
                  layout="3-big-top", total=13.90, t_ins=None,
                  captions=[
...
def _caption_img(text, max_w, size, red, seed):
    """Parchment caption box (RGBA) -- same look as the approved _text_layer
    boxes (wobbled ink border + drop shadow), sized to its text."""
...
def render_page(page_key: str, wipe_from: Path | None) -> Path:
    ...
    # cells live inside the outer page margin (comic page dress)
    m = MARGIN * SS
    ...
    # soft drop shadow per panel (precomputed once, pasted when the panel is in)
    shadows = []
    ...
    # caption images, pinned in SCREEN space at their projected position when
    # they pop in -- page-space anchoring got cropped by the camera pan (first
    # v2.1 test-gate catch); pinning at the t_in projection keeps every box
    # fully readable while still sitting on its panel
    caps = []
    WM_ZONE = (25, 55, 255, 145)   # AWAKEDEN watermark corner (screen px)
    ...
    for i in range(n_frames):
        ...
        # captions pinned in screen space (see init note), popping in on cue
        for c in caps:
            if c["t_in"] <= t <= c["t_out"]:
