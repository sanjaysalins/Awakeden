...
def torn_band(w=1900, h=260, out="caption_band.png"):
    ...
    # Mottled ink stains: ImageDraw writes RGBA pixels directly (no blending),
    # so drawing straight onto `img` with a low alpha punches near-transparent
    # holes instead of darkening the paper underneath -- they'd read as pale/
    # white blotches, not ink. Draw opaque stains on a separate layer, blur
    # it, then alpha_composite that layer over the band so the low opacity
    # actually blends with the tan paper beneath it.
    ...
    for _ in range(60):
        ...
        r = rng.uniform(4, 22)
        a = rng.randint(30, 70)
        sd.ellipse([x - r, y - r, x + r, y + r], fill=(90, 60, 30, a))
...
if __name__ == "__main__":
    ...
    torn_band(out=str(assets / "caption_band.png"))
    brush_arrow(out=str(assets / "ink_arrow.png"))
