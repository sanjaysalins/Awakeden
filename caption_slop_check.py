...
padding, or mojibake. Plain short sentences only. KJV red-letter text is exempt
from rewriting but still scanned (it should never contain these anyway).

Scans:
  - every visual/livingpage_short.spec.json beat caption (cap.text)
  - every publish/*.md TITLE/DESCRIPTION/CAPTION body
...
TOKENS = (" - ", "—", "–", "...", "…", "â€")  # " - ", em/en dash, ellipsis, mojibake
...
def scan(root: Path) -> int:
    ...
    for md in sorted(root.rglob("publish/*.md")):
        if ".claude" in md.parts or md.name.upper() == "SKILL.MD":
            continue
