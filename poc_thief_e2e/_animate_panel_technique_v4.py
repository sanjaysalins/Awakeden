"""Technique test v4 (2026-07-25): same source page and safest (v3) motion
wording, but now pass the SAME image as both --start-image and --end-image so
Kling must reconcile back to the exact source frame by the end. Tests whether
a forced start=end loop suppresses the invention drift seen in v1/v2/v3 (a
scratch mark appearing, expression shifts, invented birds) since the model
can no longer just wander off and stay there.

  .venv\\Scripts\\python.exe poc_thief_e2e/_animate_panel_technique_v4.py
"""
...
ENV_MANDATE_V3 = (
    ...
    "figure -- is NOT frozen and must show bold, clearly visible motion, with equal force to "
    "the freeze rule above. "
)
PANEL_MOTION_V4 = (
    ...
    "ALL FOUR panels must show clearly visible background motion for the middle of the clip. "
)
VARIANT_4 = (
    "A finished comic-strip page with four panels, 9:16. This clip begins on this exact frame "
    "and must end on this exact same frame -- the very first frame and the very last frame are "
    "identical, pixel-for-pixel, in every panel. The camera does not move at all. In between "
    "the start and the end, for the middle of the clip, the page comes alive: " + PANEL_MOTION_V4 +
    FIGURE_FREEZE_V3 + ENV_MANDATE_V3 +
    "By the final frame everything must have settled back to exactly match the first frame, "
    "with no new marks, no expression changes, and no new objects anywhere."
)


def main():
    dur = _hf_duration("kling3_0", 5)
    name = "technique_v4_loop_startend"
    out = OUT / f"{name}.mp4"
    cmd = [HF, "generate", "create", "kling3_0", "--start-image", str(SRC), "--end-image", str(SRC),
           "--prompt", VARIANT_4, "--duration", str(dur), "--aspect_ratio", "9:16", "--mode", "pro",
           "--sound", "off", "--wait"]
    print(f"[clip] {name} kling3_0 {dur}s (start=end) -> {out.name} ...", flush=True)
    t = time.time()
